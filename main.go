...
func main() {
	...
	//jwtMiddleware constrains the user to use 'post', 'search' and 'cluster' only if they log in
...
//objectName means the name of file stored in GCS
func saveToGCS(r io.Reader, objectName string) (string, error) {
	...
	//modify the user privilige such that every users are able to read in GCS.
...
//id is the document id in elastic search
